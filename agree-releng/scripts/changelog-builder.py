...
VALID_LABELS = ['AGREE', 'TCG', 'Simulator', 'documentation', 'releng']
...
r = http.request('GET', 'https://api.github.com/repos/loonwerks/agree/issues?milestone=2&state=closed&per_page=80')
...
for elem in data:
    for label in elem['labels']:
        if label['name'] in VALID_LABELS:
            titles[label['name']].append(elem['title'] + ' ([' + str(elem['number']) + '](https://github.com/loonwerks/agree/issues/' + str(elem['number']) + '))')
