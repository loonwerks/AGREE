...
public class AgreeJavaValidator extends AbstractAgreeJavaValidator {
	...
	public void checkAssign(AssignStatement assign) {
		...
		TypeDef lhsType = (ats.inferFromNamedElement(namedEl));
		TypeDef rhsType = (ats.infer(expr));

		if (!ats.typesEqual(lhsType, rhsType)) {
			error(assign, "The left hand side of the assignment statement is of type '" + ats.nameOfTypeDef(lhsType)
					+ "' but the right hand side is of type '" + ats.nameOfTypeDef(rhsType) + "'");
		}
	...
	public void checkArg(Arg arg) {
		...
		if (type instanceof PrimType) {
			...
			if (finalId instanceof DataImplementation) {
				if (ats.typesEqual(ats.typeDefFromType(
						recType),
				...
				if (((DataImplementation) finalId).getAllSubcomponents().size() != 0) {
					if (ats.typesEqual(ats.typeDefFromType(recType), AgreeTypeSystem.Prim.BoolTypeDef)
							|| ats.typesEqual(ats.typeDefFromType(recType), AgreeTypeSystem.Prim.IntTypeDef)
							|| ats.typesEqual(ats.typeDefFromType(recType), AgreeTypeSystem.Prim.RealTypeDef)) {
			...
			if (finalId instanceof DataType) {
				if (ats.typesEqual(ats.typeDefFromType(recType), AgreeTypeSystem.Prim.ErrorTypeDef)) {
	...
	public void checkFloorCast(FloorCast floor) {
		...
		TypeDef exprType = ats.infer(floor.getExpr());

		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
			error(floor, "Argument of floor cast is of type '" + ats.nameOfTypeDef(
					exprType)
	...
	public void checkRealCast(RealCast real) {
		...
		TypeDef exprType = ats.infer(real.getExpr());

		if (!ats.typesEqual(AgreeTypeSystem.Prim.IntTypeDef, exprType)) {
			error(real, "Argument of real cast is of type '" + ats.nameOfTypeDef(
					exprType)
...
//					}
	...
	public void checkAssume(AssumeStatement assume) {
		...
		if (expr != null) {
			TypeDef exprType = ats.infer(expr);
			if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
				error(assume, "Expression for assume statement is of type '" + ats.nameOfTypeDef(
						exprType)
	...
	public void checkInitialStatement(InitialStatement statement) {
		...
		TypeDef exprType = ats.infer(statement.getExpr());
		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
			error(statement,
					"Expression for 'initially' statement is of type '" + ats.nameOfTypeDef(
							exprType)
					+ "' but must be of type 'bool'");
		}
	}
	...
	public void checkAssert(AssertStatement asser) {
		...
		if (expr != null) {
			TypeDef exprType = ats.infer(expr);
			if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
				error(asser, "Expression for assert statement is of type '" + ats.nameOfTypeDef(
						exprType)
				+ "' but must be of type 'bool'");
			}
		}
...
//			>>>>>>> origin/develop
	...
	public void checkGuarantee(GuaranteeStatement guar) {
		...
		if (expr != null) {
			TypeDef exprType = ats.infer(expr);
			if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
				error(guar, "Expression for guarantee statement is of type '" + ats.nameOfTypeDef(
						exprType)
				+ "' but must be of type 'bool'");
			}
		}
	...
	public void checkPeriodicStatement(PeriodicStatement statement) {
		...
		TypeDef eventType = ats.infer(event);
		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, eventType)) {
	...
	public void checkWhenHoldsStatement(WhenHoldsStatement when) {
		...
		TypeDef type = ats.infer(condition);
		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, type)) {
			error(condition,
					"The condition of a when statement is of type '" + type + "'" + " but must be of type 'bool'");
		}
		type = ats.infer(event);
		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, type)) {
	...
	public void checkWhenOccursStatment(WhenOccursStatment when) {
		...
		type = ats.infer(times);
		if (!ats.typesEqual(AgreeTypeSystem.Prim.IntTypeDef, type)) {
			error(event,
	...
	public void checkTimeExpr(EObject expr, NamedElement id) {
		TypeDef type = ats.inferFromNamedElement(id);
	...
	public void checkLemma(LemmaStatement lemma) {
		...
		if (expr != null) {
			TypeDef exprType = ats.infer(expr);
			if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
				error(lemma, "Expression for lemma statement is of type '" + ats.nameOfTypeDef(
						exprType)
				+ "' but must be of type 'bool'");
			}
		}
	...
	public void checkUnaryExpr(UnaryExpr unaryExpr) {
		...
		TypeDef typeRight = ats.infer(unaryExpr.getExpr());
		String op = unaryExpr.getOp();

		switch (op) {
		case "-":
			if (!ats.typesEqual(AgreeTypeSystem.Prim.IntTypeDef, typeRight)
					&& !ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, typeRight)) {
				error(unaryExpr, "right side of unary expression '" + op + "' is of type '" + typeRight
						+ "' but must be of type 'int' or 'real'");
			}
			break;
		case "not":
			if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, typeRight)) {
				error(unaryExpr, "right side of unary expression '" + op + "' is of type '" + typeRight
						+ "' but must be of type 'bool'");
			}
			break;
	...
	public void checkPropertyStatement(PropertyStatement propStat) {
		...
		TypeDef exprType = ats.infer(propStat.getExpr());
		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
	...
	public void checkRecordUpdateExpr(RecordUpdateExpr upExpr) {
		...
		TypeDef recordType = ats.infer(upExpr.getRecord());
		...
		if (recordType instanceof RecordTypeDef) {
			...
			TypeDef keyType = ats.typeDefFromType(arg.getType());
			checkTypeExists(upExpr.getExpr());
			TypeDef expType = ats.infer(upExpr.getExpr());

			if (!ats.typesEqual(keyType, expType)) {
				error(upExpr, "the update field is of type '" + ats.nameOfTypeDef(keyType)
						+ "', but the expression is of type '" + ats.nameOfTypeDef(expType) + "'");
			}

		} else {
			error("Record to be updated must be a data implementation or AGREE record type.  " + "Found type '"
					+ ats.nameOfTypeDef(recordType) + "'.",
					upExpr,
...
//			>>>>>>> origin/develop
		...
		TypeDef t1 = ats.infer(target);
		for (Expr e : exprs) {
			checkTypeExists(e);
			TypeDef t2 = ats.infer(e);
			if (!ats.typesEqual(t1, t2)) {
				error(e, "type of element must be " + ats.nameOfTypeDef(t1) + ", but has type "
						+ ats.nameOfTypeDef(t2));
			}
	...
	public void checkArrayUpdateExpr(ArrayUpdateExpr arrup) {
		...
		TypeDef arrType = ats.infer(arrExpr);

		if (arrType instanceof ArrayTypeDef) {
			TypeDef t = ((ArrayTypeDef) arrType).stemType;
			TypeDef elmType = ats.infer(exprs.get(0));
			if (!ats.typesEqual(elmType, t)) {
				error(exprs.get(0),
						"type of element must be " + ats.nameOfTypeDef(elmType) + ", but has type "
								+ ats.nameOfTypeDef(t));
			}
	...
	public void checkArraySubExpr(ArraySubExpr asub) {
		...
		TypeDef arrType = ats.infer(arrExp);
		...
		TypeDef indexType = ats.infer(index);
		if (!ats.typesEqual(indexType, AgreeTypeSystem.Prim.IntTypeDef)) {
...
//					>>>>>>> origin/develop
		...
		for (int i = 0; i < recExpr.getArgs().size(); i++) {
			...
			TypeDef exprType = ats.infer(argExprs.get(i));

			if (!ats.typesEqual(defType, exprType)) {
...
//						>>>>>>> origin/develop
		...
		if (rec instanceof RecordDef) {
			RecordDef recDef = (RecordDef) rec;
			for (Arg arg : recDef.getArgs()) {
				typeMap.put(arg.getName(), ats.typeDefFromType(arg.getType()));
			}
		} else if (rec instanceof DataImplementation) {
			DataImplementation dataImpl = (DataImplementation) rec;
			for (Subcomponent sub : dataImpl.getAllSubcomponents()) {
				typeMap.put(sub.getName(), ats.typeDefFromClassifier((sub.getClassifier())));
			}
		} else {
			error(recType, "Record type '" + rec.getName() + "' must be a feature group or a record type definition");
		}
...
//												>>>>>>> origin/develop
	...
	public void checkConstStatement(ConstStatement constStat) {
		TypeDef expected = ats.typeDefFromType(constStat.getType());
		TypeDef actual = ats.infer(constStat.getExpr());

		if (!ats.typesEqual(expected, actual)) {
...
//	>>>>>>> origin/develop
	...
	private void checkMultiAssignEq(EObject src, List<Arg> lhsArgs, Expr rhsExpr) {
		...
		for (Arg arg : lhsArgs) {
			agreeLhsTypes.add(ats.typeDefFromType(arg.getType()));
		}
		...
		if (rhsExpr instanceof CallExpr) {
			...
			if (namedEl instanceof NodeDef) {
				...
				for (Arg var : nodeDef.getRets()) {
					agreeRhsTypes.add(ats.typeDefFromType(var.getType()));
				}
			} else if (namedEl instanceof FnDef) {
				FnDef fnDef = (FnDef) namedEl;
				agreeRhsTypes.add(ats.typeDefFromType(fnDef.getType()));
			} else {
				return; // parse error
			}
		} else {
			checkTypeExists(rhsExpr);
			TypeDef rhsType = ats.infer(rhsExpr);
		...
		for (int i = 0; i < agreeLhsTypes.size(); i++) {
			...
			if (!ats.typesEqual(rhsType, lhsType)) {
				error(src, "The variable '" + lhsArgs.get(i).getName() + "' on the left side of equation is of type '"
						+ ats.nameOfTypeDef(lhsType) + "' but must be of type '" + ats.nameOfTypeDef(rhsType) + "'");
			}
		}
...
//												>>>>>>> origin/develop
	...
	public void checkNodeLemma(NodeLemma nodeLemma) {
		TypeDef exprType = ats.infer(nodeLemma.getExpr());
		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, exprType)) {
	...
	public void checkLinearizationDefExpr(LinearizationDef linDefExpr) {
		...
		for (Arg arg : linDefExpr.getArgs()) {
			TypeDef argType = ats.typeDefFromType(arg.getType());
			if (!ats.typesEqual(argType, AgreeTypeSystem.Prim.RealTypeDef)) {
		...
		if (precisionExpr != null) {
			TypeDef precisionExprType = ats.infer(precisionExpr);
			if (!ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, precisionExprType)) {
				error(precisionExpr,
	...
	public void checkLinearizationInterval(LinearizationInterval linInterval) {
		...
		TypeDef startExprType = ats.infer(startExpr);
		TypeDef endExprType = ats.infer(endExpr);

		// The type of the interval start and end must be of real type
		if (!ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, startExprType)) {
			error(startExpr, message + ", found type " + startExprType + ".");
		}
		if (!ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, endExprType)) {
	...
	public void checkPrevExpr(PrevExpr prevExpr) {
		...
		TypeDef delayType = ats.infer(prevExpr.getDelay());
		checkTypeExists(prevExpr.getInit());
		TypeDef initType = ats.infer(prevExpr.getInit());

		if (!ats.typesEqual(initType, delayType)) {
	...
	public List<TypeDef> typeDefsFromArgs(List<Arg> args) {
		...
		for (Arg arg : args) {
			types.add(ats.typeDefFromType(arg.getType()));
		}
		return types;
	}
...
//			>>>>>>> origin/develop
		...
		for (Expr expr : call.getArgs()) {
			...
			argCallTypes.add(ats.infer(expr));
		}
		...
		for (int i = 0; i < inDefTypes.size(); i++) {
			...
			if (!ats.typesEqual(defType, callType)) {
...
//							>>>>>>> origin/develop
	...
	public void checkFnDefExpr(FnDef fnDef) {
		...
		TypeDef fnType = ats.typeDefFromType(fnDef.getType());

		TypeDef exprType = ats.infer(fnDef.getExpr());
		if (!ats.typesEqual(exprType, fnType)) {
			error(fnDef, "Function '" + fnDef.getName() + "' is of type '" + ats.nameOfTypeDef(fnType)
					+ "' but its expression is of type '" + ats.nameOfTypeDef(exprType) + "'");
		}
	...
	public void checkIfThenElseExpr(IfThenElseExpr expr) {
		...
		TypeDef condType = ats.infer(expr.getA());

		checkTypeExists(expr.getB());
		TypeDef thenType = ats.infer(expr.getB());

		checkTypeExists(expr.getC());
		TypeDef elseType = ats.infer(expr.getC());

		if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, condType)) {
			error(expr, "The condition of the if statement is of type '" + condType + "' but must be of type 'bool'");
		}

		if (!ats.typesEqual(elseType, thenType)) {
	...
	public void checkBinaryExpr(BinaryExpr binExpr) {
		...
		TypeDef typeLeft = ats.infer(binExpr.getLeft());
		TypeDef typeRight = ats.infer(binExpr.getRight());
		...
		case "->":
			if (isInLinearizationBodyExpr) {
				...
				if (!ats.typesEqual(typeRight, typeLeft)) {
					error(binExpr,
							"left and right sides of binary expression '" + op + "' are of type '"
									+ ats.nameOfTypeDef(typeLeft) + "' and '" + ats.nameOfTypeDef(
											typeRight)
		...
		case "or":
			if (isInLinearizationBodyExpr) {
				...
				if (!ats.typesEqual(AgreeTypeSystem.Prim.BoolTypeDef, typeLeft)) {
		...
		case ">=":
			if (isInLinearizationBodyExpr) {
				...
				if (!ats.typesEqual(AgreeTypeSystem.Prim.IntTypeDef, typeLeft)
						&& !ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, typeLeft)) {
					error(binExpr, "left side of binary expression '" + op + "' is of type '" + ats.nameOfTypeDef(
							typeLeft)
					+ "' but must be of type" + "'int' or 'real'");
				}
				if (!ats.typesEqual(AgreeTypeSystem.Prim.IntTypeDef, typeRight)
						&& !ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, typeRight)) {
					error(binExpr,
							"right side of binary expression '" + op + "' is of type '" + ats.nameOfTypeDef(
									typeRight)
							+ "' but must be of type" + "'int' or 'real'");
				}
			}
		...
		case "*":
			if (!ats.typesEqual(typeRight, typeLeft)) {
				error(binExpr,
						"left and right sides of binary expression '" + op + "' are of type '"
								+ ats.nameOfTypeDef(typeLeft) + "' and '" + ats.nameOfTypeDef(typeRight)
								+ "', but must be of the same type");
			}
			if (!ats.typesEqual(AgreeTypeSystem.Prim.IntTypeDef, typeLeft)
					&& !ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, typeLeft)) {
			...
			if (!ats.typesEqual(AgreeTypeSystem.Prim.IntTypeDef, typeRight)
					&& !ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, typeRight)) {
				error(binExpr, "right side of binary expression '" + op + "' is of type '" + ats.nameOfTypeDef(
						typeRight)
				+ "' but must be of type" + "'int' or 'real'");
			}
		...
		case "/":
			if (!ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, typeLeft)) {
				error(binExpr, "left side of binary expression '" + op + "' is of type '" + ats.nameOfTypeDef(
						typeLeft)
				+ "' but must be of type 'real'");
			}
			if (!ats.typesEqual(AgreeTypeSystem.Prim.RealTypeDef, typeRight)) {
				error(binExpr, "right side of binary expression '" + op + "' is of type '"
						+ ats.nameOfTypeDef(typeRight)
				+ "' but must be of type 'real'");
			}
