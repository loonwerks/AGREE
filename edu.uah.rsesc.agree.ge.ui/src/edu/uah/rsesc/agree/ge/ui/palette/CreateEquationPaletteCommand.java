...
public class CreateEquationPaletteCommand extends BasePaletteCommand implements TargetedPaletteCommand {
	public CreateEquationPaletteCommand() {
		super("Equation", AgreePaletteCategories.AGREE, null);
	}
		...
		return Optional.of(Operation.createWithBuilder(createOp -> {
			getClassifierOpBuilder().buildOperation(createOp, targetBoc.getBusinessObject())
					.map(AgreeHandlerUtil.toBusinessObjectToModify()).modifyPreviousResult(modifyBo -> {
						...
						newArg.setName(AgreeNamingUtil.buildUniqueIdentifier(agreeContract, "var"));
