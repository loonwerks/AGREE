...
public class AssertStatementHandler extends AgreeBusinessObjectHandler implements CustomRenamer {
	private static final Graphic graphic = RectangleBuilder.create().build();

	@Override
	public boolean isApplicable(final IsApplicableContext ctx) {
		return ctx.getBusinessObject(AssertStatement.class).isPresent();
	}
	...
	public String getName(final GetNameContext ctx) {
		return getName(ctx.getBusinessObject(AssertStatement.class).get());
	}

	public String getName(final AssertStatement bo) {
		return AgreeHandlerUtil.toDisplayName(bo.getStr());
	}

	@Override
	public String getNameForRenaming(final GetNameContext ctx) {
		return ctx.getBusinessObject(AssertStatement.class).get().getStr();
	}
	...
	public void rename(final RenameContext ctx) {
		ctx.getBusinessObject(AssertStatement.class).ifPresent(bo -> bo.setStr(ctx.getNewName()));
	}
}
