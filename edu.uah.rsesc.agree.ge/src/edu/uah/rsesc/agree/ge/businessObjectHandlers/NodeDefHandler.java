...
public class NodeDefHandler extends AgreeBusinessObjectHandler implements CustomRenamer {
	private static final Graphic graphic = RectangleBuilder.create().build();

	@Override
	public boolean isApplicable(final IsApplicableContext ctx) {
		return ctx.getBusinessObject(NodeDef.class).isPresent();
	}
	...
	public String getName(final GetNameContext ctx) {
		return getName(ctx.getBusinessObject(NodeDef.class).get());
	}

	public String getName(final NodeDef bo) {
		return bo.getName();
	}
	...
	public Optional<String> validateName(final RenameContext ctx) {
		return AgreeNamingUtil.validateName(ctx.getBusinessObject(NodeDef.class).get(), ctx.getNewName());
	}

	@Override
	public void rename(final RenameContext ctx) {
		ctx.getBusinessObject(NodeDef.class).ifPresent(bo -> bo.setName(ctx.getNewName()));
	}
}
