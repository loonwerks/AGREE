...
public class PropertyStatementHandler extends AgreeBusinessObjectHandler implements CustomRenamer {
	private static final Graphic graphic = RectangleBuilder.create().build();

	@Override
	public boolean isApplicable(final IsApplicableContext ctx) {
		return ctx.getBusinessObject(PropertyStatement.class).isPresent();
	}
	...
	public String getName(final GetNameContext ctx) {
		return getName(ctx.getBusinessObject(PropertyStatement.class).get());
	}

	public String getName(final PropertyStatement bo) {
		return bo.getName();
	}
	...
	public Optional<String> validateName(final RenameContext ctx) {
		return AgreeNamingUtil.validateName(ctx.getBusinessObject(PropertyStatement.class).get(), ctx.getNewName());
	}

	@Override
	public void rename(final RenameContext ctx) {
		ctx.getBusinessObject(PropertyStatement.class).ifPresent(bo -> bo.setName(ctx.getNewName()));
	}
}
