...
public class RecordDefExpressionHandler extends AgreeBusinessObjectHandler implements CustomRenamer {
	private static final Graphic graphic = RectangleBuilder.create().build();

	@Override
	public boolean isApplicable(final IsApplicableContext ctx) {
		return ctx.getBusinessObject(RecordDef.class).isPresent();
	}
	...
	public String getName(final GetNameContext ctx) {
		return getName(ctx.getBusinessObject(RecordDef.class).get());
	}

	public String getName(final RecordDef bo) {
		return bo.getName();
	}
	...
	public Optional<String> validateName(final RenameContext ctx) {
		return AgreeNamingUtil.validateName(ctx.getBusinessObject(RecordDef.class).get(), ctx.getNewName());
	}

	@Override
	public void rename(final RenameContext ctx) {
		ctx.getBusinessObject(RecordDef.class).ifPresent(bo -> bo.setName(ctx.getNewName()));
	}
}
